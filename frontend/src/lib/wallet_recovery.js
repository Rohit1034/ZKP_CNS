 ...
 */
export async function finalRecoveryStep(recoveredMasterKey, vaultBlob = null) {
  console.log('🔍 Final Recovery: Looking for encrypted wallet...')

  let encFinal = null
  // Priority: use provided vaultBlob (from server) if supplied
  if (vaultBlob) {
    console.log('🔍 Final Recovery: Using vault blob provided by server')
    encFinal = vaultBlob
  } else {
