...
export default function WalletRecoveryPage() {
  ...
  setStatus('Key reconstructed. Performing server login to fetch vault...')
  // Attempt to authenticate to server (challenge/verify) so we can fetch the server-stored vault if present
  const loginResult = await autoLogin(username, recoveredMasterKey)

  // loginResult may contain vault_blob (from server) when verifyLogin returns it
  const serverVault = loginResult && loginResult.vault_blob ? loginResult.vault_blob : null

  setStatus('Decrypting wallet with recovered key...')
  await finalRecoveryStep(recoveredMasterKey, serverVault)
  console.log('Wallet decrypted successfully')

  setRecoveredKey(recoveredMasterKey)
  setIsRecovered(true)
  setStatus('Key Recovered and wallet imported. Redirecting...')

  // After successful decryption, navigate to dashboard
  ...
  const autoLogin = async (username, recoveredMasterKey) => {
    try {
      ...
      if (result.status === 'success') {
        // Persist session for subsequent API calls
        localStorage.setItem('session_token', result.session_token)
        localStorage.setItem('current_user', username)
        localStorage.setItem('isLoggedIn', 'true')
        try { window.dispatchEvent(new Event('login-success')) } catch (e) {}
        // Return result (may include vault_blob)
        return result
      } else {
