...
import { sendSharesEmail, requestChallenge, verifyLogin, updateVault } from '../utils/api'
...
export default function WalletSetupPage() {
  ...
  const handleCreateWallet = async () => {
    ...
    try {
      ...
      if (resp.status === 'success') {
        ...
        // Persist encrypted wallet to server so recovery works from any device.
        // We'll perform an authenticated login using the derived rootKey to obtain a session token,
        // then call updateVault to store the encrypted vault blob in the user's DB record.
        try {
          setStatus('Performing secure server-side vault upload...')

          // 1. Request challenge from server
          const challenge = await requestChallenge(username)
          if (challenge.status !== 'success') {
            console.warn('Vault upload: challenge request failed', challenge)
            setStatus('Wallet created locally, but could not obtain server challenge to upload vault.')
            // Redirect to login as fallback
            setTimeout(() => navigate('/login'), 2000)
            return
          }

          // 2. Compute private scalar from rootKey and generate proof
          const { publicY, x } = await computePublicY(rootKey)
          const { R, s } = await generateProof(x, challenge.c)

          // 3. Verify login and receive session token
          const loginRes = await verifyLogin({ username, challenge_id: challenge.challenge_id, R, s })
          if (loginRes.status !== 'success') {
            console.warn('Vault upload: verifyLogin failed', loginRes)
            setStatus('Wallet created locally, but auto-login to upload vault failed. Please login manually to sync.')
            setTimeout(() => navigate('/login'), 2000)
            return
          }

          // 4. Save session token and upload vault_blob
          localStorage.setItem('session_token', loginRes.session_token)
          localStorage.setItem('current_user', username)

          // Read the encrypted wallet from localStorage where we saved it earlier
          const vaultBlobStr = localStorage.getItem('wallet_priv_final_enc')
          if (vaultBlobStr) {
            const vaultObj = JSON.parse(vaultBlobStr)
            const upd = await updateVault(vaultObj)
            setLastUploadResult(upd)
            if (upd.status === 'success') {
              setStatus('✅ Vault uploaded to server. You can recover from any device now.')
              // Redirect to dashboard after short delay
              setTimeout(() => navigate('/dashboard'), 1500)
              return
            } else {
              console.warn('Vault upload failed:', upd)
              setStatus('Wallet created but server vault upload failed. You can still recover from this device.')
              setTimeout(() => navigate('/login'), 2000)
              return
            }
          } else {
            console.warn('No local vault blob found to upload')
            setStatus('Wallet created locally but no local vault blob found to upload.')
            setTimeout(() => navigate('/login'), 2000)
            return
          }
        } catch (err) {
          console.error('Vault upload error:', err)
          setStatus('Wallet created locally, but uploading to server failed: ' + err.message)
          setLastUploadResult({ status: 'error', message: err.message })
          setTimeout(() => navigate('/login'), 2000)
        }
  ...
  // Expose manual upload so user can retry upload if automatic upload failed
  const uploadVaultManually = async () => {
    const username = localStorage.getItem('current_user') || ''
    if (!username) return setLastUploadResult({ status: 'error', message: 'No current_user in localStorage' })
    const vaultBlobStr = localStorage.getItem('wallet_priv_final_enc')
    if (!vaultBlobStr) return setLastUploadResult({ status: 'error', message: 'No local vault blob found' })
    try {
      const password = window.prompt('Enter your wallet password to derive root key and upload vault:')
      if (!password) return setLastUploadResult({ status: 'error', message: 'Upload cancelled by user' })
      setStatus('Deriving root key from provided password...')

      const salt_kdf = localStorage.getItem(`salt_kdf_${username}`)
      const kdf_params_raw = localStorage.getItem(`kdf_params_${username}`)
      if (!salt_kdf || !kdf_params_raw) return setLastUploadResult({ status: 'error', message: 'Missing KDF params in localStorage' })
      const kdf_params = JSON.parse(kdf_params_raw)

      const rootKey = await deriveRootKey(password, salt_kdf, kdf_params)
      setStatus('Requesting challenge from server...')
      const challenge = await requestChallenge(username)
      if (challenge.status !== 'success') return setLastUploadResult({ status: 'error', message: 'challenge failed', detail: challenge })

      const { x } = await computePublicY(rootKey)
      const { R, s } = await generateProof(x, challenge.c)
      setStatus('Verifying proof and obtaining session...')
      const loginRes = await verifyLogin({ username, challenge_id: challenge.challenge_id, R, s })
      if (loginRes.status !== 'success') return setLastUploadResult({ status: 'error', message: 'verifyLogin failed', detail: loginRes })

      // upload vault
      const vaultObj = JSON.parse(vaultBlobStr)
      setStatus('Uploading vault to server...')
      const upd = await updateVault(vaultObj)
      setLastUploadResult(upd)
      if (upd.status === 'success') {
        setStatus('Vault uploaded successfully.')
        localStorage.setItem('session_token', loginRes.session_token)
        localStorage.setItem('isLoggedIn', 'true')
        return upd
      } else {
        setStatus('Vault upload failed: ' + (upd.message || 'Unknown'))
        return upd
      }
    } catch (err) {
      setLastUploadResult({ status: 'error', message: err.message })
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-white p-4 relative">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Debug / Manual Upload Panel */}
        <div className="mt-4 p-4 border-2 border-gray-200 rounded-xl bg-gray-50 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold text-gray-700">Debug / Vault Upload</div>
            <div>
              <button
                onClick={uploadVaultManually}
                className="bg-blue-600 text-white px-3 py-1 rounded mr-2"
              >
                Retry Upload (enter password)
              </button>
            </div>
          </div>
          <div className="mt-3 text-xs text-gray-700">
            <div className="font-medium">Last upload result:</div>
            <pre className="whitespace-pre-wrap text-xs mt-2 bg-white p-2 rounded border border-gray-200" style={{maxHeight: 200, overflow: 'auto'}}>
              {lastUploadResult ? JSON.stringify(lastUploadResult, null, 2) : 'No upload attempted yet.'}
            </pre>
            <div className="mt-2 text-xxs text-gray-500">Use this panel to retry uploading the encrypted vault to the server so recovery works from another device.</div>
          </div>
        </div>
      </div>
    </div>
